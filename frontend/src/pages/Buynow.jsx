...
const Buynow = () => {
  ...
  const getFormattedAddress = (address) => {
    ...
    if (!addressLine || !city || !state || !pincode) return "Incomplete address";
  ...
  return (
    <>
      ...
      <div className="flex flex-col lg:flex-row mb-2 bg-gray-100 justify-between px-4 py-4 gap-4">
        ...
        <div className="w-full lg:w-3/4 px-4 rounded-md space-y-4">
          ...
          <div className="border-b bg-white p-4">
            <div className="flex justify-between items-center mb-2">
              <h1 className="font-bold text-lg">
                Delivering to {defaultAddress?.fullName || "No Address Found"}
              </h1>
              <button className="text-blue-500 text-sm" onClick={() => setShowChangeAdress(true)}>
                Change
              </button>
            </div>
            <p className="text-sm text-gray-600 mb-2">{getFormattedAddress(defaultAddress)}</p>
            <p className="text-sm text-gray-600">Phone: {defaultAddress?.phoneNumber}</p>
            <button
            ...
            <div className="space-y-6 border p-6 rounded-lg">
              ...
              <div className="border-b pb-4">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  />
                  <button className="py-1 px-3 border rounded-full text-sm">Apply</button>
                </div>
              </div>
          ...
          <div className="bg-white p-4">
            ...
            <Link to="/cart">
              <button className="text-xs text-blue-400">Back to cart</button>
            </Link>
          </div>
        </div>
        ...
        <div className="w-full h-fit lg:w-1/4 bg-white p-4 rounded-md">
          <button
            ...
            className="bg-yellow-300 hover:bg-yellow-400 w-full px-4 rounded-full mb-4 py-2 text-sm font-semibold"
          >
      ...
      <div className="bg-gray-950 py-5 text-xs text-white text-center">
      ...
      {showInstructionsModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 relative rounded-md w-full max-w-fit shadow-lg">
            ...
            <div>
              <p>{user?.name}</p>
              <p>{getFormattedAddress(defaultAddress)}</p>
              ...
              <h1 className="py-1 border bg-slate-200 px-2 mt-3">
                Can you receive deliveries at this address on weekends?
              </h1>
            </div>
            <textarea
              rows="4"
              className="w-full p-2 border rounded text-sm mt-3"
      ...
      {showChangeAddress && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 relative rounded-md w-full max-w-fit shadow-lg">
            ...
            <button
              onClick={() => setShowChangeAdress(false)}
